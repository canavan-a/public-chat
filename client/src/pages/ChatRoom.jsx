...
const ChatRoom = (props) => {
  ...
  useEffect(() => {
    ...
    ws.current = new WebSocket(
      `${wsApi}/upgrade?username=${username}&room=${room}`
    );
    ws.current.onopen = () => {
          ...
          <div className="card sm:w-[100%] lg:w-[50%] bg-base-100 shadow-xl">
            <div className="card-body">
                      ...
                      {value.conclusion === "chat" ? (
                        ...
                        >
                          {value.username === username ? (
                            ...
                            <b className="badge badge-sm mb-1">
                              {value.username}
                            </b>
                          )}
                          <div
                            className={`chat-bubble max-w-30 break-words ${
                              value.username === username
                                ? "chat-bubble-primary"
                        ...
                        <>
                          {value.conclusion === "entering" ? (
                            <div className="text-center">
                              <b>{value.username}</b> has joined
                            </div>
                          ) : (
                            <div className="text-center">
                              <b>{value.username}</b> left
                            </div>
                          )}
                        </>
                      )}
              ...
              <div className="grid grid-cols-12 gap-x-2">
                <input
                  className="input input-bordered input-success input-md col-span-10"
